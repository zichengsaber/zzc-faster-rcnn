...
class FasterRCNNVGG16(FasterRCNN):
    ...
    def __init__(self,
        ...
        extractor,classifer=decom_vgg16()
...
class VGG16RoIHead(nn.Module):
    ...
    def forward(self,x,rois,roi_indices):
        """Forward path
        We assume that there are N batch
        ...
        Args:
            ...
            roi_indices (Tensor): An array containing indices of images to
                which bounding boxes correspond to.
                Its shape is :math:`(R',)`.
        """
        roi_indices=at.totensor(roi_indices).float()
        rois=at.totensor(rois).float()
        batchindices_and_rois=torch.cat([roi_indices[:,None],rois],dim=1)
        
        # RoIPool
        pool=self.roi(x,batchindices_and_rois)
